...
// Set error reporting for development (remove or reduce in production)
...
try {
    ...
    // If you prefer to load the original HTML file and transform it,
    // you can uncomment the block below. For reliability we build a cleaned HTML below.
    /*
    if (!file_exists($htmlPath)) {
        throw new Exception("Receipt HTML file not found at: " . $htmlPath);
    }
    $rawHtml = file_get_contents($htmlPath);
    // Optionally transform rawHtml here (replace variables, grid, etc.)
    */

    // Build mPDF-friendly HTML (keeps the visual identity but uses supported CSS)
    $htmlContent = '
    <!DOCTYPE html>
    <html lang="en">
    <head>
    ...
    <body>
        ...
        <div class="receipt-container">
            ...
            <div class="date-section">
                ...
                <div class="date-value">December 29, 2025</div>
            </div>

            <div class="details-box">
                <div class="details-title">Booking Details</div>
                <table class="row-table">
                    <tr><td class="label">Payer Name:</td><td class="value">John Doe</td></tr>
                    <tr><td class="label">Hostel Name:</td><td class="value">Molyko Heights</td></tr>
                    <tr><td class="label">Booking Period:</td><td class="value">1 Year (2025-2026)</td></tr>
                </table>
            </div>
            ...
            <div class="total-section">
                <table class="row-table border-dashed">
                    <tr>
                        ...
                        <td class="value" style="font-size: 13pt;">280,000 FCFA</td>
                    </tr>
                </table>
                <table class="row-table" style="margin-top: 5pt;">
                    <tr>
                        <td class="text-brand-blue" style="font-size: 18pt; font-weight: bold;">Total Paid:</td>
                        <td class="text-brand-blue" style="font-size: 20pt; font-weight: bold; text-align: right;">280,000 FCFA</td>
                    </tr>
                </table>
            </div>
...
<svg class="icon-svg" width="20" height="20" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg" aria-hidden="true" focusable="false">
  <path fill="#3b82f6" d="M20.52 3.48A11.93 11.93 0 0 0 12 0C5.37 0 .01 5.36.01 12c0 2.11.55 4.18 1.6 6.01L0 24l6.2-1.62A11.94 11.94 0 0 0 12 24c6.63 0 12-5.36 12-12 0-1.98-.48-3.86-1.48-5.5zM12 21.5c-1.2 0-2.38-.28-3.44-.82l-.25-.12-3.68.96.98-3.6-.13-.27A9.5 9.5 0 0 1 2.5 12 9.5 9.5 0 0 1 12 2.5c5.24 0 9.5 4.26 9.5 9.5S17.24 21.5 12 21.5zM16.1 14.6c-.26-.13-1.54-.76-1.78-.85-.24-.09-.42-.13-.6.13-.18.26-.7.85-.86 1.03-.16.18-.32.2-.59.07-.27-.13-1.14-.42-2.17-1.34-.8-.71-1.34-1.59-1.5-1.86-.16-.27-.02-.42.12-.55.12-.12.27-.32.4-.48.13-.16.17-.27.26-.45.09-.18.04-.34-.02-.47-.06-.13-.6-1.44-.82-1.97-.22-.52-.45-.45-.62-.46-.16-.01-.35-.01-.54-.01s-.45.06-.69.34c-.24.28-.92.9-.92 2.2 0 1.3.94 2.56 1.07 2.74.13.18 1.85 2.9 4.49 3.95 3.14 1.24 3.14.83 3.7.78.56-.05 1.82-.74 2.08-1.45.26-.71.26-1.32.18-1.45-.08-.13-.24-.18-.5-.31z"/>
    ...
    if ($isCLI) {
        // Save to file when running from CLI
        $filePath = $projectRoot . '/receipt.pdf';
        // clear any output buffers before writing to file
        while (ob_get_level()) {
            ob_end_clean();
        }
        $mpdf->Output($filePath, 'F');
        echo "✅ PDF generated: $filePath\n";
        exit;
    } else {
        // Force download in browser: clear buffers then send as download once
        while (ob_get_level()) {
            ob_end_clean();
        }
